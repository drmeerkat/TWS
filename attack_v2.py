...
def PGD(model, 
    ...
    for i in range(allstep):
        ...
        if istargeted:
            ...
            loss1 = -cross_entropy(output_ori, original_softmax) # untargeted attack, loss of original softmax, should ascend
        ...
        if setting == 'white':
            ...
            if istargeted:
                ...
                new_target = torch.LongTensor([random_label(output_ori.data.max(1, keepdim=True)[1][0].item(), dataset = dataset)]).cuda()
            ...
            if istargeted:
                ...
                loss4 = F.cross_entropy(output_u_attack, torch.LongTensor([output_ori.data.max(1, keepdim=True)[1][0].item()]).cuda())
            total_loss -= loss4
...
def CW(model, 
    ...
    for step in range(allstep):
        ...
        if istargeted:
            ...
            if argmax11 == true_label:
                argmax11 = top2_1[0][1]
            loss1 = -(output_ori[0][argmax11] - output_ori[0][true_label] + margin).clamp(min=0)
        ...
        if setting == 'white':
            ...
            if istargeted:
                ...
                output_ori_label = output_ori.data.max(1, keepdim=True)[1][0].item()
                new_tl = random_label(output_ori_label, dataset=dataset)
            ...
            if istargeted:
                ...
                if argmax14 == output_ori_label:
                    argmax14 = top2_4[0][0]
                loss4 = (output_u_attack[0][argmax14] - output_u_attack[0][output_ori_label] + margin).clamp(min = 0) 
            total_loss -= loss4
...
def BIM(model, 
    ...
    for step in range(allstep):
        ...
        output_ori = model(transform(x_var.clone(), dataset=dataset))
        loss1 = F.cross_entropy(output_ori, true_label) #untargeted attack, loss of true label, should ascend

        if setting == "white":
            total_loss += lbd * loss1  # loss of original image, should ascend
            ...
            raise "attack setting not supported"

        total_loss.backward()
        x_var += torch.clamp(alpha*x_var.grad.data.sign(), min=-epsilon, max=epsilon)
        x_var.data = torch.clamp(x_var, min=0, max=1)
