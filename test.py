...
def tune_criterion_thresholds(model,
                              ...
                              untargeted_lr,
                              u_radius):
    target_1 = l1_vals(model, loader, "Clean", n_radius)
    target_2 = targeted_vals(model, loader, "Clean", targeted_lr, t_radius)
    target_3 = untargeted_vals(model, loader, "Clean", untargeted_lr, u_radius)
    p_1 = target_1.copy()
    p_2 = target_2.copy()
    p_3 = target_3.copy()
    p_1.sort()
    ...
    np.save('clean_val1', target_1)
    np.save('clean_val2', target_2)
    np.save('clean_val3', target_3)
    np.save('fpr', fpr)
...
if __name__ == "__main__":
    ...
    model_root = os.path.join(args.model_dir, 'classifier_{}_{}_best.pth'.format(args.dataset, args.arch))
    if args.arch == "cwnet" or args.arch == "vgg11bn":
        classifier = eval(args.arch)(num_classes=10)
        classifier.load_state_dict(torch.laod(model_root)))
    elif args.arch == "wrnet":
        classifier = wide_resnet(depth=28, num_classes=10, widen_factor=10) # only for cifar10
        classifier = load_GPUS(classifier, model_root)
    else:
        raise Exception("Unknown model type")

    # record the detail of this exp in str?
    exp_name = '{}_{}'.format(args.dataset, args.type)
    ...
    p_3.sort()
    fpr = np.zeros(len(p_1)*len(p_2)*len(p_3)+1)
    for ix_1 in tqdm(range(0,len(p_1)), desc='Clean'):
        for ix_2 in range(0,len(p_2)):
            for ix_3 in range(0,len(p_3)):
                fpr[len(p_2)*len(p_3)*ix_1+len(p_3)*ix_2+ix_3] = len(target_1[np.logical_or(np.logical_or(target_1>p_1[ix_1],target_2>p_2[ix_2]),target_3>p_3[ix_3])])*1.0/len(target_1)
    fpr[-1] = len(target_1[np.logical_or(np.logical_or(target_1>=p_1[-1],target_2>=p_2[-1]),target_3>=p_3[-1])])*1.0/len(target_1)
    ...
    for i, attack in enumerate(attacks):
        ...
        for ix_1 in tqdm(range(0,len(p_1)), desc=attacks[i]):
            for ix_2 in range(0,len(p_2)):
                for ix_3 in range(0,len(p_3)):
                    tpr[len(p_2)*len(p_3)*ix_1+len(p_3)*ix_2+ix_3] = len(a_target_1[np.logical_or(np.logical_or(a_target_1>p_1[ix_1],a_target_2>p_2[ix_2]),a_target_3>p_3[ix_3])])*1.0/len(a_target_1)
    ...
    print("Results for %s is "%exp_name, results)
